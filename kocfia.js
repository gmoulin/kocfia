...
//CSS rules declarations
	...
	var overviewCss = "#kocfia-overview { position:absolute; font: 13px/20px Verdana, sans serif; font-width: normal;  z-index: 100000; display: none; }";
		...
		overviewCss += "\n#kocfia-overview .inCamps { font-size: 10px; }";
		...
		overviewCss += "\n#kocfia-overview.ui-draggable .wrap { margin: 0 10px 10px 10px; width: auto; }";
	...
	var summaryCss = "#kocfia-summary { padding: 2px 3px; }";
		summaryCss += "\n#kocfia-summary .title { margin: 0 }";
		...
		summaryCss += "\n#kocfia-summary .wrapper { overflow: auto; margin: 0 10px 10px 10px; width: auto; text-align: left; }";
		summaryCss += "\n#kocfia-summary .content { padding: 5px 20px 5px 5px; }";
		summaryCss += "\n#kocfia-summary .content img { width: 16px; position: relative; top: 4px; }";
		summaryCss += "\n#kocfia-summary .content table { width: 100%; }";
		summaryCss += "\n#kocfia-summary .content th { text-align: center; }";
		summaryCss += "\n#kocfia-summary .content th, #kocfia-summary .content td { padding: 0 2px; }";
		summaryCss += "\n#kocfia-summary .content tbody td { text-align: right; padding-right: 4px; }";
...
(function(window, document, $, undefined){
	...
	var $head = $('head'),
		...
		resetRaidInterval, autoFormationInterval,
	...
	KOCFIA.init = function(){
		...
		if( window.location.host.indexOf('kocfia.kapok.') == -1 ){
		...
		KOCFIA.confPanel = function(){
			...
			for( i = 0; i < modulesLength; i += 1 ){
				...
				if( typeof KOCFIA[ mod ].modPanel == 'function' ){
					...
					var name = ( KOCFIA.tabLabel[ mod ] ? KOCFIA.tabLabel[ mod ] : mod.capitalize() );
	...
	/* SHARED */
		...
		/* march */
			...
			Shared.getUnitLimit = function( cityKey, hasAuraBoost, hasAura2Boost ){
				if( KOCFIA.debug && KOCFIA.debugWhat.hasOwnProperty('shared') ) console.info('KOCFIA shared getUnitLimit function', attack);
			...
			Shared.cancelMarch = function( marchId, cityKey, type, attempts ){
				...
				params.marchId = marchId.replace(/m/, '');
				...
				.done(function(result){
					if( result.ok ){
						if( result.updateSeed ){
							...
							window.seed.queue_atkp[ cityKey ][ marchId ].destinationUnixTime = (result.hasOwnProperty('destinationUnixTime') ? parse(result.destinationUnixTime, 10) : timestamp);
							window.seed.queue_atkp[ cityKey ][ marchId ].returnUnixTime = (result.hasOwnProperty('returnUnixTime') ? parse(result.returnUnixTime, 10) : timestamp + marchtime * parseInt(result.returnMultiplier));
							window.seed.queue_atkp[ cityKey ][ marchId ].marchStatus = 8;
		...
		/* knight */
			Shared.getKnightStatText = function( knight ){
				...
				if( knight.resourcefulness == knight.politics && knight.resourcefulness == knight.combat && knight.resourcefulness == knight.intelligence ){
					...
					return 'ingéniosité '+ stat;
				} else if( knight.politics == stat ){
					return 'politique '+ stat;
		...
		/* players */
			...
			Shared.getDiplomacy = function( allianceId ){
				...
				if( window.seed.allianceDiplomacies === null || $.isEmptyObject(window.seed.allianceDiplomacies) ){
					return 'neutral';
				} else if( window.seed.allianceDiplomacies.hasOwnProperty('friendly') && window.seed.allianceDiplomacies.friendly.hasOwnProperty( 'a' + allianceId ) ){
					return 'friendly';
				} else if( window.seed.allianceDiplomacies.hasOwnProperty('hostile') && window.seed.allianceDiplomacies.hostile.hasOwnProperty( 'a' + allianceId ) ){
					return 'hostile';
	...
	/* OVERVIEW */
		...
		KOCFIA.overview.on = function(){
			...
			dataTable += '</colgroup>' + headers + '<tbody>' + sizer;
		...
		KOCFIA.overview.updateFromSeed = function(){
			...
			var $popTrs = KOCFIA.overview.$tbodyTrs.filter('.population').filter(':not(:first)'),
				...
				base, total, nbLine, d, timestamp, effectGrowth, effectBase,
		...
		KOCFIA.overview.calcInnerSizes = function( size ){
			...
			var tableW = size.width - (KOCFIA.overview.movable ? 20 : 0); /* wrap margin */
			KOCFIA.overview.$wrap.css('width', tableW);
			KOCFIA.overview.$tbody.css('width', tableW);

			KOCFIA.overview.$cityTds.eq(0).css('width', 20);
			KOCFIA.overview.$cityTds.eq(1).css('width', 100);
			KOCFIA.overview.$cityTds.eq(2).css('width', 50);

			KOCFIA.overview.$cityTds.filter(':gt(2)').css('width', KOCFIA.overview.getCityColWidth( tableW ) + 'px');

			KOCFIA.overview.$headersThs.each(function(i){
				$(this).css('width', KOCFIA.overview.$cityTds.eq(i).css('width'));
			});
		};
	...
	/* AutoAttack shared base */
		...
		KOCFIA.autoAttack.on = function(){
			...
			if( KOCFIA.conf[ this.module ].automatic ){
				...
				KOCFIA[ this.module ].$nextBar.hide();
			}
		};
		...
		KOCFIA.autoAttack.attackInfo = function( attack ){
			...
			code += '<div><button class="charge button"><span>Lancer</span></button></div>';
			code += '<button class="edit button secondary"><span>Modifier</span></button>';
			code += '<div><button class="duplicate button secondary" title="Créer une nouvelle configuration à partir de celle-ci"><span>Copier</span></button></div>';
			code += '<div><button class="delete button danger"><span>Supprimer</span></button></div>';
			code += '</td>';
			...
			var knights = window.seed.knights[ attack.cityKey ],
				j, k, wavesLength = attack.waves.length, unitsLength, unitsCode, wave, unit;
			...
			KOCFIA.$confPanel.find('#kocfia-'+ this.module)
					...
					if( attack ){
						...
						for( i = 0; i < wavesLength; i += 1 ){
							...
							if( wave.units.length > 1 ){
								for( j = 1; j < wave.units.length; j += 1 ){
				...
				.on('click', '.stop', function(){
					if( confirm('Etes-vous sûr ?') ){
						...
						$this.toggleClass('stop trash secondary danger')
							.find('span').html('Enlever').attr('title', 'Supprime le suivi de cette configuration');
		...
		KOCFIA.autoAttack.modPanel = function(){
			...
			if( this.module == 'darkForest' ){
				...
				KOCFIA[ this.module ].$nextBar = $section.find('.nextIteration');
			}
	...
	/* DARK FOREST */
		...
		KOCFIA.darkForest.getForm = function(){
			...
			form += '<button class="launch" style="'+ (KOCFIA.conf.darkForest.automatic ? 'display: none;' : '') +'">Lancer</button>';
			form += '</div>';
			...
			for( i = 0; i < length; i += 1 ){
				var cityKey = KOCFIA.citiesKey[i],
					city = KOCFIA.cities[cityKey];
				form += '<tbody data-city="'+ cityKey +'"><tr class="city-header"><td colspan="99">';
				form += '<div><label for="kocfia-'+ this.module +'-'+ cityKey +'-rps">Laisser&nbsp;:&nbsp;</label>';
				form += '<input id="kocfia-'+ this.module +'-'+ cityKey +'-rps" type="number" class="rallypointSlot" required min="0" max="11" value="0"> place(s) dans le point de ralliement';
				form += '</div><button class="add-rule">Ajouter une règle</button>';
				form += '<input id="kocfia-'+ this.module +'-'+ cityKey +'" name="city" value="'+ cityKey +'" type="checkbox" class="city-active" autocomplete="off" title="Ville active ?">';
				form += '<label for="kocfia-'+ this.module +'-'+ cityKey +'" title="Ville active ?">'+ city.label +'</label>';
				form += '</td></tr><tr><td class="keep"><div>';
				form += '<span class="ui-icon ui-icon-plusthick add-unit"></span><strong>Troupes à conserver</strong>';
				form += '<span class="ui-icon ui-icon-copy copy-keep" title="Applique cette règle de conservation dans les autres villes"></span>';
				form += '</div></td></tr></tbody>';
			}
			form += '</table>';

			var levelSkel = '<td class="rule"><div>';
			levelSkel += '<span class="ui-icon ui-icon-copy copy-rule" title="Ajoute cette règle dans les autres villes"></span>';
			levelSkel += '<span class="ui-icon ui-icon-trash remove-rule"></span>';
			levelSkel += '<input type="checkbox" class="level-active" autocomplete="off" title="règle active ?">';
			levelSkel += '<label>Niveau&nbsp;:&nbsp;</label>';
			levelSkel += '<input type="number" class="targetLevel" required min="1" max="10" placeholder="1"></div>';
			...
			unitSkel += '<select class="unit-choice" autocomplete="off" required>';
			unitSkel += '<option value="">Choisir</option>';
			unitSkel += '</select>';
			unitSkel += '<input class="unit-qty" type="text" autocomplete="off" required pattern="'+ Shared.numberRegExp +'" placeholder="1.2k">';
			unitSkel += '</div>';
			...
			waveSkel += '<span class="ui-icon ui-icon-minusthick remove"></span>';
			waveSkel += '</div>';
		...
		KOCFIA.darkForest.getListsTemplate = function(){
			...
			onGoing += '<span class="nextIteration"></span>';
		...
		KOCFIA.darkForest.addSectionListeners = function(){
			...
			KOCFIA.$confPanel.find('#kocfia-'+ this.module)
				.on('change', '#'+ module +'-panel-automatic', function(){
					var checked = $(this).prop('checked');
					$('#'+ module +'-automatic').prop('checked', checked).change();
					if( checked ) KOCFIA[ module ].$accordion.accordion('activate', false).accordion('activate', 1);
				...
				//copy rule
				.on('click', '.copy-rule', function(){
					var $this = $(this),
						$tbodies = $this.closest('tbody').siblings('tbody'),
						$rule = $this.closest('.rule'),
						$inputs = $rule.find('input, select');

					$tbodies.find('tr').filter(':not(.city-header)').each(function(){
				...
				//dupplicate keep
				.on('click', '.copy-keep', function(){
					var $this = $(this),
						$keeps = $this.closest('tbody').siblings('tbody').find('.keep'),
						$keep = $this.closest('.keep'),
						$inputs = $keep.find('input, select');

					$keeps.each(function(){
						var $clone = $keep.clone();
						$clone.find('input, select').each(function(i){
							var $el = $(this);
							if( $el.is(':checkbox') ){
								$el.prop('checked', $inputs.eq(i).prop('checked'));
				...
				.on('click', '.add-unit', function(){
					...
					if( $td.hasClass('keep') ){
						$clone.find('input, select').removeAttr('required');
					}
	...
	/* SUMMARY */
		...
		KOCFIA.summary.on = function(){
			...
			var code = '<h3 class="title">Résumé</h3><div class="wrapper"><div class="content">';
			...
			//hapiness
			code += '<tr><td>Bonheur</td>'+ cels +'</tr>';

			//total training
			code += '<tr><td title="Total de la durée des formations">Formations</td>'+ cels +'</tr>';
			...
			//total stone
			code += '<tr><td title="Stock '+ KOCFIA.resourceInfo.rec3.labelBis + KOCFIA.resourceInfo.rec3.label +'">Stock <img src="'+ KOCFIA.resourceInfo.rec3.icon +'"></td>'+ cels +'</tr>';

			//total ore
			code += '<tr><td title="Stock '+ KOCFIA.resourceInfo.rec4.labelBis + KOCFIA.resourceInfo.rec4.label +'">Stock <img src="'+ KOCFIA.resourceInfo.rec4.icon +'"></td>'+ cels +'</tr>';
			...
			$summary
				...
				.resizable({
					...
					handles: 'n, e, s, w, ne, se, sw, nw',
					resize: function(){
						KOCFIA.summary.$wrapper.css('height', KOCFIA.summary.calcInnerHeight());
					},
					stop: function(event, ui){
			...
			KOCFIA.summary.$wrapper = KOCFIA.summary.$div.find('.wrapper');
			...
			$summaryToggle.click(function(){
				...
				if( KOCFIA.conf.summary.visible ) KOCFIA.summary.$wrapper.css('height', KOCFIA.summary.calcInnerHeight());
		...
		KOCFIA.summary.calcInnerHeight = function(){
			return KOCFIA.summary.$div.innerHeight() - KOCFIA.summary.$title.height() - 15;
		};
		...
		KOCFIA.summary.update = function(){
			...
			var i, b, l, length, knight, barrackModifier, geometry, infantryModifier, nb, queue, keys,
				nbBarracks, barracksLevelSum, barrackLevel, workshopLevel, stableLevel, blacksmithLevel, marshalCombat,
				popPerHour, hapiness, unt2Time, unt8Time, unt12Time, unt2PerHour, unt8PerHour, unt12PerHour,
				timestamp = Date.timestamp();
			...
			for( i = 0, length = KOCFIA.citiesKey.length; i < length; i += 1 ){
				...
				for( b in window.seed.buildings[ cityKey ] ){
					if( window.seed.buildings[ cityKey ].hasOwnProperty(b) ){
						...
						if( building[0] == 13 ){
							nbBarracks += 1;
							barracksLevelSum += parseInt(building[1], 10);
							if( building[1] > barrackLevel ) barrackLevel = parseInt(building[1], 10);
						} else if( building[0] == 15 ){
							blacksmithLevel = parseInt(building[1], 10);
				...
				barrackModifier = nbBarracks + 0.1 * (barracksLevelSum - nbBarracks);
				geometry = parseInt(window.seed.tech.tch5, 10);
				infantryModifier = 1 + marshalCombat / 200 + geometry;

				unt2Time = (barrackLevel > 0 ? 50 / (barrackModifier * infantryModifier) / 2 : 0);

				unt8Time = (barrackLevel > 6 && blacksmithLevel > 4 && stableLevel > 4 && parseInt(window.seed.tech.tch12, 10) > 4 ? 1500 / (barrackModifier * (infantryModifier + stableLevel / 10)) : 0);

				unt12Time = (barrackLevel > 9 && stableLevel > 1 && workshopLevel > 8 && fletching > 9 && parseInt(window.seed.tech.tch13, 10) > 9 ? 6000 / (barrackModifier * (infantryModifier + stableLevel / 10) + workshopLevel / 10) : 0);

				unt2PerHour = unt2Time > 0 ? 3600 / unt2Time : 0;
				unt8PerHour = unt8Time > 0 ? 3600 / unt8Time : 0;
				unt12PerHour = unt12Time > 0 ? 3600 / unt12Time : 0;

				unt2Delta = popPerHour - unt2PerHour;
				unt8Delta = popPerHour - unt8PerHour;
				unt12Delta = popPerHour - unt12PerHour;
				...
				KOCFIA.summary.$tds.eq(i + 1 + KOCFIA.summary.numCelsPerLine * 0).html( Shared.format(unt2PerHour) );
				KOCFIA.summary.$tds.eq(i + 1 + KOCFIA.summary.numCelsPerLine * 1).html( Shared.format(unt8PerHour) );
				KOCFIA.summary.$tds.eq(i + 1 + KOCFIA.summary.numCelsPerLine * 2).html( Shared.format(unt12PerHour) );

				KOCFIA.summary.$tds.eq(i + 1 + KOCFIA.summary.numCelsPerLine * 3).html( Shared.format(popPerHour) );

				KOCFIA.summary.$tds.eq(i + 1 + KOCFIA.summary.numCelsPerLine * 4).html( Shared.format(unt2Delta) );
				KOCFIA.summary.$tds.eq(i + 1 + KOCFIA.summary.numCelsPerLine * 5).html( Shared.format(unt8Delta) );
				KOCFIA.summary.$tds.eq(i + 1 + KOCFIA.summary.numCelsPerLine * 6).html( Shared.format(unt12Delta) );

				KOCFIA.summary.$tds.eq(i + 1 + KOCFIA.summary.numCelsPerLine * 7).html( hapiness );

				KOCFIA.summary.$tds.eq(i + 1 + KOCFIA.summary.numCelsPerLine * 8).html( Shared.readableDuration(time) );
				KOCFIA.summary.$tds.eq(i + 1 + KOCFIA.summary.numCelsPerLine * 9).html( Shared.format(nb) );

				KOCFIA.summary.$tds.eq(i + 1 + KOCFIA.summary.numCelsPerLine * 10).html( Shared.format(stone) );
				KOCFIA.summary.$tds.eq(i + 1 + KOCFIA.summary.numCelsPerLine * 11).html( Shared.format(ore) );


				sums.unt2PerHour += unt2PerHour;
				sums.unt8PerHour += unt8PerHour;
				sums.unt12PerHour += unt12PerHour;

				sums.popPerHour += popPerHour;

				sums.unt2Delta += unt2Delta;
				sums.unt8Delta += unt8Delta;
				sums.unt12Delta += unt12Delta;
			...
			for( b in sums ){
				if( sums.hasOwnProperty(b) ){
					console.log(b, sums[b], i * KOCFIA.summary.numCelsPerLine + KOCFIA.summary.numCelsPerLine - 1);
					if( b != 'trainTime' ){
						KOCFIA.summary.$tds.eq(i * KOCFIA.summary.numCelsPerLine + KOCFIA.summary.numCelsPerLine - 1).html( sums[ b ] > 0 ? Shared.format(sums[ b ]) : '' );
					} else {
						KOCFIA.summary.$tds.eq(i * KOCFIA.summary.numCelsPerLine + KOCFIA.summary.numCelsPerLine - 1).html( sums[ b ] > 0 ? Shared.readableDuration(sums[ b ]) : '' );
					}
					i += 1;
				}
			}
		...
		KOCFIA.map.modPanel = function(){
			...
			$section.append( code )
				...
				.on('click', '.map-category', function(){
					...
					if( !$.isEmptyObject(KOCFIA.map.currentSearch) ){
									...
									//city
										if( tile.tileType == 51 ){
											if( tile.tileCityId !== null ){
												...
												tiles.city.push({ s: 'C', r: range, x: tile.xCoord, y: tile.yCoord, m: Shared.format(user.m), u: tile.tileUserId, p: name, g: alliance, a: user.a, c: tile.cityName, b: 0 });
												coords[ tile.xCoord +','+ tile.yCoord ] = 1;
											} else {
									//barbarian
	...
	/* FORMATION */
		...
		KOCFIA.formation.modPanel = function(){
			...
			//formations list
				...
				onGoing += '<span class="nextIteration"></span>';
				onGoing += '<button class="button secondary history-toggle" title="Historique des formations"><span>Historique</span></button>';
				...
				onGoing += '<div class="unit-toggles buttonset">'+ toggles +'</div>';
				...
				onGoing += '<button class="button secondary history-toggle" title="Historique des fortifications"><span>Historique</span></button>';
				onGoing += '</h3><div class="formation-list ongoing">';
				onGoing += '<div class="fort-toggles buttonset">'+ toggles +'</div>';
				onGoing += '<table class="fort"><thead><tr>'+ headers +'<th>Suivi</th></tr></thead>';
				onGoing += '<tbody><tr>'+ cels +'<td class="info"></td></tr></tbody></table></div>';

			var help = KOCFIA.formation.getHelp();
			var history = '<div id="kocfia-formation-history" class="history unit" title="Historique des formations"></div>';
			history += '<div id="kocfia-formation-history" class="history fort" title="Historique des fortifications"></div>';
				...
				.on('click', '.save', function(){
					...
					if( result.errors.length ){
						...
						var name = $.trim( KOCFIA.formation.$ruleName.val() ),
							...
							ruleName;
						if( name !== '' && KOCFIA.formation.savedRules.hasOwnProperty( name ) ){
	...
	/* TRANSPORT */
		...
		KOCFIA.transport.confPanel = function( $section ){
			...
			code += Shared.generateCheckbox('transport', 'automatic', 'Lancer les transports automatiques', KOCFIA.conf.transport.automatic);
			code += Shared.generateButton('transport', 'deleteSupply', 'Supprimer toutes les règles d\'approvisionnement  enregistrées');
			code += Shared.generateButton('transport', 'deletePileUp', 'Supprimer toutes les règles de stockage enregistrées');
			code += '</div>';
		...
		KOCFIA.transport.modPanel = function(){
			...
			code += '<button class="button secondary"><span>Aide</span></button>';
			code += '</div><h3>Configurations</h3>';
			code += '<div class="accordion">';
			code +=  manualForm + pileUpForm + supplyForm;
			code += '</div><div>'+ onGoing +'</div>';
			code += help;
			code += '<div id="kocfia-transport-history-pileUp" class="history pileUp" title="Historique des approvisionnements"></div>';
			code += '<div id="kocfia-transport-history-supply" class="history supply" title="Historique des stockages"></div>';
		...
		KOCFIA.transport.automaticOn = function(){
			...
			Shared.nextIteration( KOCFIA.transport.$nextPileUpBar, 30 );
			window.setTimeout(function(){
		...
		KOCFIA.transport.automaticOff = function(){
			...
			/*Shared.stopProgress( KOCFIA.transport.$nextPileUpBar );
			Shared.stopProgress( KOCFIA.transport.$nextSupplyBar );*/
		};
		...
		KOCFIA.transport.getListsTemplate = function(){
			...
			form += '<span class="nextIteration pileUp"></span>';
			...
			form += '<span class="nextIteration supply"></span>';
		...
		KOCFIA.transport.getHelp = function(){
			...
			help += '<li>Pour chaque ressource vous pouvez ajouter des règles de transport</li>';
			...
			help += '<li>Pour chaque couple expéditeur / destinataire il y aura autant de marches que de troupes différentes (3 règles avec wagon, 4 avec cavalerie équivaut à 2 marches)</li>';
			help += '<li>Utiliser la fonction de dupplication au niveau d\'une règle va la duppliquer pour toutes les villes manquantes (ormis la destination) pour la ressource</li>';
			help += '<li>Utiliser la fonction de dupplication dans la colonne ressource va prendre les règles de la ressource et les duppliquer dans les autres ressources en écrasant celles déjà présente</li>';
			...
			help += '<li>Si les transports automatiques sont actifs, les modifications seront prise en compte dès la prochaine tentative de transport</li>';
			...
			for( i = 0; i < KOCFIA.resources.length; i += 1 ){
				...
				if( inSeed ){
					...
					if( n > 0 ){
						...
						code += '<img src="'+ type.icon +'" alt="'+ type.label +'"><span>'+ Shared.format(n) +'</span></button>';
					}
				}
			}
			return code;
			...
			KOCFIA.transport.$manualForm
				...
				.on('change', '.items input', function(){
					...
					//unit quantity limit
						...
						$('#kocfia-transport-item-286').prop('checked', false);
					} else if( val == '286' ){
						$('#kocfia-transport-item-285').prop('checked', false);
					}
		...
		KOCFIA.transport.getAutoForm = function(type){
			...
			form += '</div><div class="result"></div>';
			form += '<table><thead><tr>';
			form += '<th>Ressources</th><th>Règles</th>';
			form += '</tr></thead><tbody>';
		...
		KOCFIA.transport.addAutoFormListeners = function(){
			if( KOCFIA.debug && KOCFIA.debugWhat.hasOwnProperty('transport') ) console.info('KOCFIA transport addPileUpFormListerners function');
			...
			KOCFIA.transport.$autoForms
				...
				.on('click', '.save', function(){
					var $form = $(this).closest('.transport-form'),
						type = ($form.hasClass('pileUp') ? 'pileUp' : 'supply'),
						$msg = $form.find('.result').empty(),
		...
		KOCFIA.transport.planAutomaticTransport = function(type){
			...
			var $resources = KOCFIA.transport.$autoForms.filter('.'+ type).find('tbody').find('tr'),
				$tr, city, i, unit, $rule, r, res, rule, from, to, qty, coord, label, tmp,
				senderCoord = null,
				receiverCoord = null,
				plan = {},
				errors = [],
			...
			$resources.each(function(){
				...
				$tr.find('.rule').each(function(i){
					label = 'pour '+ res.label +' '+ (i === 0 ? '1ère' : (i + 1) +'ème') +' règle';
					$rule = $(this);
					...
					if( isPileUp ){
						...
						tmp.active = $rule.find('.active').prop('checked');
						plan[ r ][ from ].push(tmp);
					}
					...
					for( from in plan[ r ] ){
						if( plan[ r ].hasOwnProperty( from ) ){
							...
							if( isPileUp ){
								...
								for( i = 0; i < plan[ r ][ from ].length; i += 1 ){
									tmp = plan[ r ][ from ][ i ];

									to = (tmp.to === null ? tmp.coord.join(',') : tmp.to);
									if( !finalPlan[ from ].hasOwnProperty( to ) ) finalPlan[ from ][ to ] = {};
		...
		KOCFIA.transport.launchAutomaticTransport = function(type){
			...
			if( type == 'pileUp' ){
				KOCFIA.transport.$ongoingPileUp.empty();
			} else {
				KOCFIA.transport.$ongoingSupply.empty();
			}
			...
			if( !$.isEmptyObject( KOCFIA.transport[ type ] ) ){
				...
				var sequence = function(){
					if( KOCFIA.debug && KOCFIA.debugWhat.hasOwnProperty('transport') ) console.info('kocfia transport launchAutomaticTransport deferred sequence function');
				...
				var checkRules = function(dfd){
					...
					maxUnitPerSlot = Shared.getUnitLimit( cityKey, false, false );
					...
					for( j = 0; j < KOCFIA.resources.length; j += 1 ){
						...
						if( rules.hasOwnProperty( resKey ) ){
							...
							if( type == 'pileUp' ){
								...
								transportQuantity = availableResource - rule.quantity;
								...
								if( destResource < rule.quantity ){
									transportQuantity = rule.quantity - destResource;
							...
							minTroop = KOCFIA.transport.getTroopQuantityForLoad( [{id: currentUnit, qty: 1}], transportQuantity, loadBoost );
							...
							if( minTroop > unitLeft ){
								...
								resourcesQuantity = KOCFIA.transport.getLoadCapacity( [{id: currentUnit, qty: unitLeft}], loadBoost );
	...
	/* ALARM */
		...
		KOCFIA.alarm.confPanel = function( $section ){
			...
			code += Shared.generateCheckbox('alarm', 'playSoundForScout', 'Jouer un son pour chaque éclairage', KOCFIA.conf.alarm.playSoundForScout);
			code += Shared.generateCheckbox('alarm', 'playSoundForAutonomy', 'Jouer un son pour les autonomies', KOCFIA.conf.alarm.playSoundForAutonomy);
			code += Shared.generateCheckbox('alarm', 'playSoundForAllianceReport', 'Jouer un son pour les rapports d\'alliance sur joueurs non connectés', KOCFIA.conf.alarm.playSoundForAllianceReport);
			code += '<br><p>Les transports, attaques et raids seront bloqués tant que la ville est attaquée</p>';
		...
		KOCFIA.alarm.off = function(){
			...
			if( KOCFIA.conf.alarm.watchAllianceReports ){
				if( watchReportsInterval ) window.cleanInterval( watchReportsInterval );
			}
		};
			...
			if( KOCFIA.conf.alarm.active && !Array.isArray(window.seed.queue_atkinc) ){
				...
				for( m in window.seed.queue_atkinc ){
					if( window.seed.queue_atkinc.hasOwnProperty(m) ){
						...
						if( $.inArray(m, KOCFIA.alarm.incomming) == -1 && (march.marchType == 3 || march.marchType == 4) ){
							KOCFIA.alarm.incomming.push(m);
							cityKey = 'city'+ march.toCityId;
							KOCFIA.alarm.underAttack( cityKey );
		...
		KOCFIA.alarm.checkCityStatus = function( cityKey ){
			if( KOCFIA.debug && KOCFIA.debugWhat.hasOwnProperty('alarm') ) console.info('KOCFIA alarm checkCityStatus function');
		...
		KOCFIA.alarm.react = function(type, city, info){
			...
			if( type == 'allianceReport' ){
				//check if user is online
			} else {
				if( KOCFIA.conf.alarm[ 'playSoundFor'+ type.capitalize ] ){
				...
				if( KOCFIA.conf.alarm.postToAlliance ){
					...
					switch( type ){
						case 'autonomy':
							...
							message = '<!-- kocfia alarm autonomy -->'+ city.name +' '+ Shared.mapLink(city.coords.x +','+ city.coords.y) +' a besoin '+ resInfo.labelBis + resInfo.label +' ! ';
						...
						case 'attack':
							...
							var knights = window.seed.knights[ cityKey ],
								...
								marshallMsg = '', combat;
							if( knights && marshallId ){
								var marshall = knigts[ 'knt'+ marshallId ],
									timestamp = Date.timestamp();
								combat = marshall.combat;

								marshallMsg += ' | maréchal '+combat;
							...
							if( info.hasOwnProperty('fromXCoord') ){
								attacker += ' depuis '+ Shared.mapLink( info.fromXCoord +','+ info.fromYCoord );
							}
							if( info.hasOwnProperty('knt') && info.knt.hasOwnProperty('cbt') ){
								combat = parseInt(info.knt.cbt, 10);
								if( !isNaN(combat) ){
									attacker += ' (chevalier '+ combat +')';
								}
							}

							var target;
							if( city.tileId == info.toTileId ){
								target = city.name +' '+ Shared.mapLink(city.coords.x +','+ city.coords.y);
							} else {
								target = 'TS '+ Shared.mapLink(city.coords.x +','+ city.coords.y);
							}

							message = '<!-- kocfia alarm attack -->'+ target +' attaquée | ';
							message += parseInt(window.seed.citystats[ cityKey ].gate, 10) === 1 ? 'défense' : 'sancturaire';
							message += ' | empennage '+ parseInt(window.seed.tech.tch13, 10) + marshallMsg + attacker;
							break;
						default:
							return;
					}

					KOCFIA.alarm.postToChat( message );
				}
			}
		};

		KOCFIA.alarm.postToChat = function(message){
			if( KOCFIA.debug && KOCFIA.debugWhat.hasOwnProperty('alarm') ) console.info('KOCFIA alarm postToChat function');

		};

		KOCFIA.alarm.updateCityState = function( cityKey ){
			if( KOCFIA.debug && KOCFIA.debugWhat.hasOwnProperty('alarm') ) console.info('KOCFIA alarm updateCityState function');

			if( KOCFIA.alarm.checkCityStatus( cityKey ) ){
				KOCFIA.alarm.underAttack[ cityKey ] = 1;

				window.setTimeout(function(){
					KOCFIA.alarm.updateCityState( cityKey );
				}, 30 * 1000);
			} else if( KOCFIA.alarm.underAttack.hasOwnProperty(cityKey) ){
				delete KOCFIA.alarm.underAttack[ cityKey ];
			}
		};
	...
	/* KNIGHTS */
		...
		KOCFIA.knights.modPanel = function(){
			...
			code += '<th>Niveau</th>';
			code += '<th>Salaire</th>';
			code += '<th>À attribuer</th>';
			code += '<th>Politique</th>';
			code += '<th>Combat</th>';
			code += '<th>Intelligence</th>';
			code += '<th>Ingéniosité</th>';
			code += '<th>Actions</th>';
			code += '<th>Boosts</th>';
			code += '</tr><thead>';

			for( i = 0, l = KOCFIA.citiesKey.length; i < l; i += 1 ){
				cityKey = KOCFIA.citieyKey[ i ];
				city = KOCFIA.cities[ cityKey ];
			...
			$section.append( header + code )
				...
				.on('click', '.update', KOCFIA.knights.update)
				.on('click', '.promote, .unpromote', function(){
					var $this = $(this),
						...
						position = parseInt(this.value, 10),
						flag = $this.hasClass('promote');

					$.when( KOCFIA.knight.promote(cityKey, knightKey, position, flag) )
						.done(function(){
							$tbody.html( $(KOCFIA.knight.byCity(cityKey)).html() );
						})
						.fail(function(){
							var $tds = $tr.find('td');

							$tds.eq(9).find('span').remove();
							$('<span>Erreur</span>').appendTo( $tds.eq(9)).fadeIn(300, function(){
								window.setTimeout(function(){
									$tds.eq(9).find('span').fadeOut(500, function(){ $(this).remove(); })
								});
							});
						});
				})
				.on('click', '.fire', function(){
					...
					$.when( KOCFIA.knight.dismiss(cityKey, knightKey) )
						.done(function(){
							$tr.remove();
						})
						.fail(function(){
				...
				.on('clic', '.stat', function(){
					var $this = $(this),
						$tr = $this.closest('tr'),
						cityKey = $tr.closest('tbody').data('city'),
						knightKey = $tr.data('knight'),
						$tds = $tr.find('td'),
						points = parseInt($tds.eq(4).html(), 10),
						stats = [
							parseInt($tds.eq(5).html(), 10),
							parseInt($tds.eq(6).html(), 10),
							parseInt($tds.eq(7).html(), 10),
							parseInt($tds.eq(8).html(), 10)
						];

					if( $this.hasClass('p') ) stats[0] += points;
					else if( $this.hasClass('c') ) stats[0] += points;
					else if( $this.hasClass('i') ) stats[0] += points;
					else if( $this.hasClass('r') ) stats[0] += points;

					$.when( KOCFIA.knight.skillUp(cityKey, knightKey, stats, points) )
						.done(function(){
							$tr.replaceWith( KOCFIA.knight.updateKnight(cityKey, knightKey) );
						})
						.fail(function(){
							$tds.eq(9).find('span').remove();
							$('<span>Erreur</span>').appendTo( $tds.eq(9)).fadeIn(300, function(){
				...
				.on('click', '.boost', function(){
					...
					$.when( KOCFIA.knight.boost(cityKey, knightKey, itemKey, ($this.hasClass('xp') ? 'xp' : 'stat')) )
						.done(function(){
							$tr.replaceWith( KOCFIA.knight.updateKnight(cityKey, knightKey) );
						})
						.fail(function(){
		...
		KOCFIA.knights.update = function(){
			...
			for( i = 0, l = KOCFIA.citiesKey.length; i < l; i += 1 ){
				...
				$code = $( KOCFIA.knights.byCity( cityKey ) );
				KOCFIA.knights.$tbodies.html( $code.html() );
			}
		};
			...
			for( k in knights ){
				if( knights.hasOwnProperty(k) ){
					...
					if( knight.knightId != leaders.politicsKnightId
						...
						code += KOCFIA.knights.updateKnight( cityKey, k ); //k = 'kntXXXX'
					}
				}
			}
		...
		KOCFIA.knights.byKnight = function( cityKey, knightKey ){
			...
			code += '<tr class="'+ (skillPointsLeft > 0 ? 'skillPointsLeft' : '' ) +'">';
			code += '<td>';
			...
			if( knight.knightId != leaders.politicsKnightId ){
				...
				code += 'Alchimiste';
				isLeader = true;
			} else if( knight.knightId == leaders.skillPointsApplied ){
				code += 'Régisseur';
				isLeader = true;
			} else if( knight.knightStatus == 10 ) code += 'en marche';
			else code += '&nbsp;';
			...
			code += '<td class="p '+ pc +' '+ (max == knight.politics ? 'primary' : '') +'">'+ p +'</td>'; //politic
			code += '<td class="c '+ cc +' '+ (max == knight.combat ? 'primary' : '') +'">'+ c +'</td>'; //combat
			code += '<td class="i '+ ic +' '+ (max == knight.intelligence ? 'primary' : '') +'">'+ i +'</td>'; //intelligence
			code += '<td class="r '+ rc +' '+ (max == knight.resourcefulness ? 'primary' : '') +'">'+ r +'</td>'; //resourcefulness
			...
			for( i = 0; i < items.length; i += 1 ){
				...
				if( parseInt(window.seed.items[ key ], 10) > 1 ){
					info = window.itemlist[ key ];
					code += '<button class="button secondary boost xp" rel="'+ items[i] +'" title="'+ info.name + ' - '+ info.description +'">';
					code += '<span><img src="'+ window.stimgUrl +'img/items/70/'+ items[i] +'.jpg"></span></button>';
				}
			}
			...
			for( i = 0; i < items.length; i += 1 ){
				...
				if( parseInt(window.seed.items[ key ], 10) > 1 ){
					info = window.itemlist[ key ];
					code += '<button class="button secondary boost stat" rel="'+ items[i] +'" title="'+ info.name + ' - '+ info.description +'">';
					code += '<span><img src="'+ window.stimgUrl +'img/items/70/'+ items[i] +'.jpg"></span></button>';
				}
			}
		...
		KOCFIA.knights.skillUp = function(cityKey, knightKey, stats, points){
			if( KOCFIA.debug && KOCFIA.debugWhat.hasOwnProperty('knights') ) console.info('KOCFIA knights skillUp function');
			return $.Deferred(function(dfd){
				var params = $.extend({}, window.g_ajaxparams),
				...
				var request = function( attempts ){
					...
					.done(function(data){
						if( data.ok ){
							window.seed.knights[cityKey][knightId].politics = stats[0];
							window.seed.knights[cityKey][knightId].combat = stats[1];
							window.seed.knights[cityKey][knightId].intelligence = stats[2];
							window.seed.knights[cityKey][knightId].resourcefulness = stats[3];
							seed.knights["city" + currentcityid][knightKey].skillPointsApplied = (parseInt(knight.skillPointsApplied, 10) + points).toString();
		...
		KOCFIA.knights.promote = function(cityKey, knightKey, position, flag){
			if( KOCFIA.debug && KOCFIA.debugWhat.hasOwnProperty('knights') ) console.info('KOCFIA knights promote function');
			return $.Deferred(function(dfd){
				var params = $.extend({}, window.g_ajaxparams),
					knightId = knightKey.replace(/knt/, '');
				params.cid = cityKey.replace(/city/, '');
		...
		KOCFIA.knights.dismiss = function(cityKey, knightKey){
			if( KOCFIA.debug && KOCFIA.debugWhat.hasOwnProperty('knights') ) console.info('KOCFIA knights dismiss function');
			return $.Deferred(function(dfd){
				var params = $.extend({}, window.g_ajaxparams),
					knightId = knightKey.replace(/knt/, '');
				params.cid = cityKey.replace(/city/, '');
		...
		KOCFIA.knights.boost = function(cityKey, knightKey, itemKey, type){
			if( KOCFIA.debug && KOCFIA.debugWhat.hasOwnProperty('knights') ) console.info('KOCFIA knights boost function');
			return $.Deferred(function(dfd){
				var params = $.extend({}, window.g_ajaxparams),
					knightId = knightKey.replace(/knt/, ''),
					...
					.done(function(data){
						if( data.ok ){
							if( type == 'stat' ){
								...
								if( itemKey == 'i221' ) window.seed.knights[ cityKey ][ knightKey ].combatBoostExpireUnixtime = data.expiration.toString();
								if( itemKey == 'i231' ) window.seed.knights[ cityKey ][ knightKey ].intelligenceBoostExpireUnixtime = data.expiration.toString();
								if( itemKey == 'i241' ) window.seed.knights[ cityKey ][ knightKey ].resourcefulnessBoostExpireUnixtime = data.expiration.toString();
							} else {
						...
						if( result.data ){
							...
							if( info ){
								...
								if( mod == 'wilderness' ){
									...
									} else if( info.tileUserId !== null || info.tileCityId !== null || info.misted ){ //"0" -> under mists, "xxx" -> no mists
											...
											//check units
												...
												for( w = 0; w < attack.waves.length; w += 1 ){
													...
													for( j = 0; j < wave.units.length; j += 1 ){
														...
														if( available < qty ){
															//KOCFIA[ mod ].refreshOngoingInfo(attack, false, 'Pas assez de troupe ('+ window.unitcost[ unit.id ][0] +') dans '+ city.label +'.');
															enoughUnits = false;
														} else {
															for( k = 0; k < attack.keep.length; k += 1 ){
																keep = attack.keep[k];
																if( unit.id == keep.id && available - qty < parseFloat(keep.qty) ){
																	//KOCFIA[ mod ].refreshOngoingInfo(attack, false, 'Pas assez de troupe ('+ window.unitcost[ unit.id ][0] +') (conservation) dans '+ city.label +'.');
																	enoughUnits = false;
																}
															}
														}
					...
					/* deferred wave specific functions */
						...
						var checkUnits = function(wdfd){
								...
								for( j = 0; j < wave.units.length; j += 1 ){
									...
									qty = parseFloat(unit.qty);
									wParams[ unitKey ] = qty;
									unitsArr[ unit.id.replace(/unt/, '') ] = qty;

								available = parseFloat(units[ unit.id ]);
								if( available < qty ){
									KOCFIA[ mod ].refreshOngoingInfo(attack, false, (mod == 'scout' ? city.label + ' ' : '') +'Pas assez de troupe ('+ KOCFIA.unitInfo[ unit.id ].label +').');
									return wdfd.reject();
								} else {
									for( k = 0; k < kLength; k += 1 ){
										keep = attack.keep[k];
										if( unit.id == keep.id && available - qty < parseFloat(keep.qty) ){
											KOCFIA[ mod ].refreshOngoingInfo(attack, false, (mod == 'scout' ? city.label + ' ' : '') +'Pas assez de troupe ('+ KOCFIA.unitInfo[ unit.id ].label +') (conservation).');
											return wdfd.reject();
										}
									}
								}
							...
							$.ajax({
								...
								.done(function( result ){
									if( result.ok ){
										...
										if( result.msg ){
											KOCFIA[ mod ].refreshOngoingInfo(attack, false, (mod == 'scout' ? city.label + ' ' : '') +'Plan d\''+ label +' sur '+ Shared.mapLink( coords[ attack.coordIndex ] ) +' refusé ('+ result.msg + (result.msg.indexOf('marais') > -1 ? ' (le serveur a retourné un type de coordonnées pas à jour)' : '') +').');
											...
											KOCFIA[ mod ].refreshOngoingInfo(attack, false, (mod == 'scout' ? city.label + ' ' : '') +'Plan d\''+ label +' sur '+ Shared.mapLink( coords[ attack.coordIndex ] ) +' refusé (captcha !).');
											return wdfd.reject();
											...
											KOCFIA[ mod ].refreshOngoingInfo(attack, false, (mod == 'scout' ? city.label + ' ' : '') +'Plan d\''+ label +' sur '+ Shared.mapLink( coords[ attack.coordIndex ] ) +' refusé (erreur serveur).');
											launchAttempts -= 1;
											if( launchAttempts > 0 ){
												return wdfd.pipe( launchWave(wdfd, launchAttempts) );
											} else {
								...
								.fail(function(){
									//network or server error
									KOCFIA[ mod ].refreshOngoingInfo(attack, false, (mod == 'scout' ? city.label + ' ' : '') +'Plan d\''+ label +' sur '+ Shared.mapLink( coords[ attack.coordIndex ] ) +' refusé (erreur internet).');
