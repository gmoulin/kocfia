...
(function(window, document, $, undefined){
	...
	/* SHARED */
		...
		/* march */
			...
			Shared.getDuration = function( cityKeyFrom, cityKeyTo, coord, unit ){
				if( KOCFIA.debug && KOCFIA.debugWhat.hasOwnProperty('shared') ) console.info('KOCFIA shared getDuration function');
				var coordXTo = null,
					coordYTo = null,
					city, distance;
	...
	/* DARK FOREST */
		...
		KOCFIA.darkForest.getBuildsList = function(){
			...
			builds += '<button class="button secondary" rel="2" title="Miliciens<br>Empennage 10 - Chevalier 140<br>pertes : ~ 4k miliciens"><span>FO2</span></button>';
			...
			builds += '<button class="button secondary" rel="3rd" title="Point de ralliement 11<br>Toutes infantries, Cavalier, Cavalier Lourd, Wagon<br>(majorité de Miliciens)<br>Empennage 10 - Chevalier 150<br>pertes : ~ 2.5k miliciens et quelques autres"><span>FO3</span></button>';
			...
			builds += '<button class="button secondary" rel="1" title="Milicien + Archers<br>Empennage 10 - Chevalier 55<br>pertes : ~ 1 milicien"><span>FO1</span></button>';
						...
						if( cityIndex >= KOCFIA.citiesKey.length ){
							...
							for( c in filtered ){
								if( filtered.hasOwnProperty(c) ){
									g = filtered[ c ];
									blocks.push("bl_" + ( g.x >= 750 ? g.x - 750 : g.x ) + "_bt_" + ( g.y >= 750 ? g.y - 750 : g.y ));
								}
							}
		...
		KOCFIA.darkForest.addSectionListeners = function(){
			KOCFIA.$confPanel.find('#kocfia-darkForest')
				...
				.on('click', '.builds .button', function(){
					...
					$form.find('div').filter(':not(.units)').remove();
	...
	/* SUMMARY */
		...
		KOCFIA.summary.update = function(){
			...
			var i, b, l, t, length, knight, barrackModifier, geometry, infantryModifier, nb, queue, keys,
				...
				timestamp = Date.timestamp(),
				throneBoost = 1 + (window.cm.ThroneController.effectBonus(77) / 100);
			...
			for( i = 0, length = KOCFIA.citiesKey.length; i < length; i += 1 ){
				...
				unt2Time = (barrackLevel > 0 ? KOCFIA.formation.unitTrainDuration(cityKey, 'unt2', 1, 9) : 0);

				unt8Time = (barrackLevel > 6 && blacksmithLevel > 4 && stableLevel > 4 && parseInt(window.seed.tech.tch12, 10) > 4 ? KOCFIA.formation.unitTrainDuration(cityKey, 'unt8', 1, 9) : 0);

				unt12Time = (barrackLevel > 9 && stableLevel > 1 && workshopLevel > 8 && parseInt(window.seed.tech.tch5, 10) > 9 && parseInt(window.seed.tech.tch13, 10) > 9 ? KOCFIA.formation.unitTrainDuration(cityKey, 'unt12', 1, 9) : 0);
	...
	/* FORMATION */
		...
		KOCFIA.formation.unitTrainDuration = function( cityKey, unit, qty, speed ){
			...
			//speed = 9 for summary panel, no max and rounding wanted
			if( speed == 9 ){
				time = time / modifier;
			} else {
			...
			//speed = 9 for summary panel, no rounding wanted
			if( speed == 9 ){
				return time / modifier / (1 + (window.cm.ThroneController.effectBonus(77) / 100));
	...
	/* TRANSPORT */
		...
		KOCFIA.transport.modPanel = function(){
			...
			code += '<span class="buttonset"><input type="checkbox" id="transport-panel-automatic-pileUp" '+ (KOCFIA.conf.transport.pileUp ? 'checked' : '') +' autocomplete="off" />';
			code += '<label for="formation-panel-automatic-pileUp">Stockages automatiques</label>';
			code += '<input type="checkbox" id="transport-panel-automatic-supply" '+ (KOCFIA.conf.transport.supply ? 'checked' : '') +' autocomplete="off" />';
			code += '<label for="formation-panel-automatic-supply">Approvisionnements automatiques</label></span>';
		...
		KOCFIA.transport.addManualFormListeners = function(){
			...
			KOCFIA.transport.$manualForm
				...
				.on('click', '.reset', function(){
					...
					KOCFIA.transport.$manualForm.find('.troop, .resources').empty();
		...
		KOCFIA.transport.launchAutomaticTransport = function(type){
			...
			if( type == 'pileUp' ){
				KOCFIA.transport.$ongoingPileUp.find('div').each(function(){
					$(this).appendTo( KOCFIA.transport.$pileUpHistory );
				});
				KOCFIA.transport.$ongoingPileUp.empty();
			} else {
				KOCFIA.transport.$ongoingSupply.find('div').each(function(){
					$(this).appendTo( KOCFIA.transport.$supplyHistory );
				});
	...
	/* REASSIGN */
		...
		KOCFIA.reassign.modPanel = function(){
			...
			code += '<label for="formation-panel-automatic">réassignements automatiques</label></span>';
			code += '<button class="button secondary help-toggle"><span>Aide</span></button>';
			code += '</div><h3>Configurations</h3>';
			code += '<div class="accordion">';
		...
		KOCFIA.reassign.automaticOn = function(){
			...
			Shared.nextIteration( KOCFIA.reassign.$nextBar, 15 );
			window.setTimeout(function(){
			...
			KOCFIA.reassign.$manualForm
				...
				.bind('getAndSetKnights', function(){
					...
					if( knights.length ){
						...
						$available.removeAttr('disabled').empty().append( code );
					} else {
						$available.attr('disabled', 'disabled').empty().append('<option value="">Aucun chevalier disponible</option>');
					}
				})
				.bind('getAndSetTroops', function(){
				...
				//form reset
				.on('click', '.reset', function(){
					KOCFIA.reassign.$manualForm.find('.troop').empty();
					KOCFIA.reassign.$manualForm.find('.knight').find('.available').empty().hide();
		...
		KOCFIA.reassign.loadAutoRuleset = function(){
			...
			if( !$.isEmptyObject( KOCFIA.reassign.rules ) ){
				var u, unitRules, cityKey, dest, $tr, $addRule, $rules,
					$units = KOCFIA.reassign.$autoForms.filter('.'+ type).find('tbody').find('tr');
		...
		KOCFIA.reassign.launchAutomaticReassign = function(type){
			...
			KOCFIA.reassign.$ongoingfind('div').each(function(){
				$(this).appendTo( KOCFIA.reassign.$history );
			});
			KOCFIA.reassign.$ongoing.empty();
			...
			if( !$.isEmptyObject( KOCFIA.reassign.rules ) ){
				...
				var rules, rule;
				//units
				var unitQuantity, availableUnit, unitInfo, unitSummary, maxUnitPerSlot, totalUnits;
				//request
				var params, unitsArr, resources;
				//loops index
				var cityIndex = 0,
				...
				var fromCity = function(dfd){
					...
					//recipients
					destinations = KOCFIA.reassign[ type ][ cityKey ];
					//get the keys of destination, for easier looping
					destinationsKeys = Object.keys(destinations);
					destinationIndex = 0;
				...
				var toCity = function(dfd){
					...
					//recipients loop finished
					if( destinationIndex >= destinationsKeys.length ){
						KOCFIA.reassign.refreshOnGoing('Vérifications pour '+ city.label +' finies.', type);
						cityIndex += 1;
						return dfd.pipe( fromCity(dfd) );
					}
					...
					if( KOCFIA.conf.alarm.active && KOCFIA.conf.alarm.stopAutoReassign && KOCFIA.alarm.underAttack.hasOwnPropery( destKey ) ){
						KOCFIA.reassign.refreshOnGoing(currentDestination +' est attaquée, réassignement annulé.', type);
						destinationIndex += 1;
						return dfd.pipe( toCity(dfd) );
					}
				...
				var checkRules = function(dfd){
					if( KOCFIA.debug && KOCFIA.debugWhat.hasOwnProperty('reassign') ) console.info('kocfia reassign launchAutomaticreassign deferred checkRules function', currentUnit);
					...
					if( totalUnits > 0 ){
						...
						for( unit in unitsArr ){
							if( unitsArr.hasOwnProperty(unit) && unitArr[ unit ] > 0 ){
								unitInfo = KOCFIA.unitInfo[ 'unt'+ j ];
								unitsSummary.push('<img src="'+ unitInfo.icon +'" alt="'+ unitInfo.label +'" title="'+ unitInfo.label +' '+ Shared.readable( unitArr[ unit ] ) +'"> '+ Shared.format( unitArr[ unit ] ));
							}
						}

						return dfd.pipe( launch(dfd, params, 3) );
					} else {
						KOCFIA.reassign.refreshOnGoing('Rien à stocker via '+ unitInfo.label +' depuis '+ city.label +'.');
						destinationIndex += 1;
						return dfd.pipe( toCity(dfd) );
					}
				};
				...
				var launch = function( dfd, tParams, attempts ){
					...
					.done(function(data){
						if( data.ok ){
							...
							KOCFIA.reassign.refreshOnGoing('Réassignement envoyé : '+ unitSummary.join(', ') +' depuis '+ city.label +' vers '+ currentDestination +'.');
								...
								if( data.user_action == 'marchWarning' ){
									...
									if( attempts > 0 ) dfd.pipe( launch(dfd, tParams, attempts) );
										...
										KOCFIA.reassign.refreshOnGoing('Réassignement refusé (serveur, action utilisateur requise) : '+ unitSummary.join(', ') +' depuis '+ city.label +' vers '+ currentDestination +'.');
										destinationIndex += 1;
										return dfd.pipe( toCity(dfd) );
									}
								}
								...
								//{"ok":false,"error_code":4,"feedback":"3-7-0","msg":"Unable to dispatch march. Not enough units"}
								KOCFIA.reassign.refreshOnGoing('Réassignement refusé ('+ data.msg +') : '+ unitSummary.join(', ') +' depuis '+ city.label +' vers '+ currentDestination +'.');
								//detect message to pipe to the correct function (not enough units, not enough resource, PR full, ...)
								destinationIndex += 1;
								return dfd.pipe( toCity(dfd) );
							} else {
								KOCFIA.reassign.refreshOnGoing('Réassignement refusé (serveur) : '+ unitSummary.join(', ') +' depuis '+ city.label +' vers '+ currentDestination +'.');
								destinationIndex += 1;
								return dfd.pipe( toCity(dfd) );
							}
						})
						.fail(function(){
							...
							if( attempts > 0 ) dfd.pipe( launch(dfd, tParams, attempts) );
								...
								KOCFIA.reassign.refreshOnGoing('Réassignement refusé (internet) : '+ unitSummary.join(', ') +' depuis '+ city.label +' vers '+ currentDestination +'.');
								destinationIndex += 1;
								return dfd.pipe( toCity(dfd) );
							}
						});
	...
	/* DATA AND STATS */
		...
		KOCFIA.dataAndStats.modPanel = function(){
			...
			data += '<th colspan="6">Statistiques</th>';
			data += '</tr><tr>';
			...
			data += '<th>Vie</th>';
			data += '<th title="Attaque">Att</th>';
			data += '<th title="Défence">Déf</th>';
			data += '<th title="Vitesse">Vit</th>';
			data += '<th title="Portée">Por</th>';
			data += '<th title="Capacité de charge">Cha</th>';
			data += '</tr></thead><tbody>';

			var i, unitKey, fortKey, unit,
				fortmight = {
					frt53: "4",
					frt55: "7",
					frt60: "1",
			...
			for( i = 0; i < KOCFIA.troops.length; i += 1 ){
				...
				data += '<th>'+ window.unitcosts[ unitKey ][5] +'</th>';
				data += '<th>'+ window.unitcosts[ unitKey ][1] +'</th>';
				data += '<th>'+ window.unitcosts[ unitKey ][2] +'</th>';
				data += '<th>'+ window.unitcosts[ unitKey ][3] +'</th>';
				data += '<th>'+ window.unitcosts[ unitKey ][4] +'</th>';
				data += '<th>'+ window.unitcosts[ unitKey ][6] +'</th>';
				data += '<th>'+ window.unitstats[ unitKey ][0] +'</th>';
				data += '<th>'+ window.unitstats[ unitKey ][1] +'</th>';
				data += '<th>'+ window.unitstats[ unitKey ][2] +'</th>';
				data += '<th>'+ window.unitstats[ unitKey ][3] +'</th>';
				data += '<th>'+ window.unitstats[ unitKey ][4] +'</th>';
				data += '<th>'+ window.unitstats[ unitKey ][5] +'</th>';
				data += '</tr>';
			}
			...
			for( i = 0; i < KOCFIA.defenses.length; i += 1 ){
				...
				data += '<tr>';
				data += '<th title="'+ unit.label +'"><img src="'+ unit.icon +'"></th>';
				data += '<th>'+ window.fortcost[ unitKey ][5] +'</th>';
				data += '<th>'+ window.fortcost[ unitKey ][1] +'</th>';
				data += '<th>'+ window.fortcost[ unitKey ][2] +'</th>';
				data += '<th>'+ window.fortcost[ unitKey ][3] +'</th>';
				data += '<th>'+ window.fortcost[ unitKey ][4] +'</th>';
				data += '<th>'+ window.fortcost[ unitKey ][6] +'</th>';
				data += '<th>'+ window.fortstats[ fortKey ][0] +'</th>';
				data += '<th>'+ window.fortstats[ fortKey ][1] +'</th>';
				data += '<th>'+ window.fortstats[ fortKey ][2] +'</th>';
				data += '<th>'+ window.fortstats[ fortKey ][3] +'</th>';
				data += '<th>'+ window.fortstats[ fortKey ][4] +'</th>';
				data += '<th>'+ window.fortstats[ fortKey ][5] +'</th>';
				data += '</tr>';
			}
			data += '</tbody></table>';

			data += '<br><br>';
			data += '<table><thead><tr><th colspan="5">Entretien et Puissance</th>';
			data += '</tr><tr>';
			data += '<th>&nbsp;</th>';
			data += '<th title="'+ KOCFIA.resourceInfo['rec0'].label +'"><img src="'+ KOCFIA.resourceInfo['rec0'].icon +'"></th>';
			...
			for( i = 0; i < KOCFIA.troops.length; i += 1 ){
				...
				data += '<tr>';
				data += '<th title="'+ unit.label +'">1 <img src="'+ unit.icon +'"></th>';
				data += '<th>'+ window.unitupkeeps[ unitKey ] +'</th>';
				data += '<th>'+ window.unitmight[ unitKey ] +'</th>';
				data += '<th>'+ window.unitcosts[ unitKey ][7] +'</th>';
				data += '<th>'+ (3600 / window.unitcosts[ unitKey ][7] * window.unitmight[ unitKey ]) +'</th>';
				data += '</tr>';
			}
			data += '</tbody></table>';

			var stats = KOCFIA.dataAndStats.update();

			$section.append( header + data + stats )
				.on('click', '.update', function(){
					$(this).closest('table').replaceWith( KOCFIA.dataAndStats.update() );
				});
		};
		...
		KOCFIA.dataAndStats.compute = function(){
			...
			var cityKey, city, i, b, stats, building,
				timestamp = Date.timestamp(),
				marshall, foreman,
				barrackMod, combatMod, politicMod, geometryMod, stableMod, blacksmithMod, giantStrenghtMod,
				infantryBonus, cavaleryBonus, siegeBonus, defenseBonus, throneBonus, boost;
			for( i = 0; i < KOCFIA.citiesKey.length; i += 1 ){
				cityKey = KOCFIA.citiesKey[ i ];
				city = KOCFIA.cities[ cityKey ];
				...
				for( b in window.seed.buildings[ cityKey ] ){
					if( window.seed.buildings[ cityKey ].hasOwnProperty(b) ){
						...
						if( building[0] == 13 ){
							...
							stats.sumLvlBarracks += lvl;
							if( lvl > stats.highestBarrack ) stats.highestBarrack = lvl;
						} else if( building[0] == 16 ){
							stats.blacksmith = parseInt(building[0], 10);
						} else if( building[0] == 15 ){
							stats.workshop = parseInt(building[0], 10);
						} else if( building[0] == 17 ){
							stats.stable = parseInt(building[0], 10);
						} else if( building[0] == 18 ){
							stats.wall = parseInt(building[0], 10);
						}
					}
				}
...
Training Time FormulaeEdit
	...
	city.Troop1Time = ((city.maxBarracks > 0)?((50/boost)/isf):0);
	city.Troop2Time = city.Troop1Time/2;
	city.Troop3Time = ((city.maxBarracks > 1 && city.eagleEyesLevel > 0)?((100/boost)/isf):0);
	city.Troop4Time = ((city.maxBarracks > 1 && city.poisonedEdgeLevel > 0)?((150/boost)/isf):0);
	city.Troop5Time = ((city.maxBarracks > 2 && city.workshopLevel > 0 && city.metalAlloysLevel > 0)?((225/boost)/isf):0);
	city.Troop6Time = ((city.maxBarracks > 3 && city.fletchingLevel > 0)?((350/boost)/isf):0);
	city.Troop7Time = ((city.maxBarracks > 4 && city.stableLevel > 0 && city.alloyHorseshoesLevel > 0)?((500/boost)/csf):0);
	city.Troop8Time = ((city.maxBarracks > 6 && city.workshopLevel > 4 && city.stableLevel > 4 && city.alloyHorseshoesLevel > 4)?((1500/boost)/csf):0);
	city.Troop9Time = ((city.maxBarracks > 5 && city.stableLevel > 0 && city.blacksmithLevel > 2 && city.featherweightPowderLevel > 0)?((1000/boost)/ssf):0);
	city.Troop10Time = ((city.maxBarracks > 7 && city.stableLevel > 1 && city.blacksmithLevel > 4 && city.geometryLevel > 4 && city.fletchingLevel > 5)?((3000/boost)/ssf):0);
	city.Troop11Time = ((city.maxBarracks > 8 && city.workshopLevel > 4 && city.stableLevel > 2 && city.blacksmithLevel > 6 && city.metalAlloysLevel > 7 && city.geometryLevel > 6)?((4500/boost)/ssf):0);
	city.Troop12Time = ((city.maxBarracks > 9 && city.stableLevel > 1 && city.blacksmithLevel > 8 && city.geometryLevel > 9 && city.fletchingLevel > 9)?((6000/boost)/ssf):0);
	// Ce n'est pas bon - les temps estimés sur les défenses remparts
